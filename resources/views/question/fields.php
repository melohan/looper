...
if (isset($params['exercise'])) {
    if ($params['exercise']->getTitle() != null) {
        $text = "Exercice: ";
        $useLink = true;
        $textLink = $params['exercise']->getTitle();
        $urlLink = '/question/fields/' . $params['exercise']->getId();
    } else {
        $text = "New exercise";
    }
} else {
    ...
    <?php if (isset($exercise) && !empty($exercise)) : ?>
        <section class="column">
            ...
            <a data-confirm="Are you sure? You won&#39;t be able to further edit this exercise" class="button"
               rel="nofollow" data-method="changeStatus" data-href="/exercise/update"
               data-val="<?= $exercise->getid() ?>" data-status="2"><i class="fa fa-comment"></i>
